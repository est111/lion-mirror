...
class StorageController extends Controller
{
    ...
    public function expoAction($id)
    {
        ...
        $query = $em->createQuery('SELECT p as product, COUNT(i.id) as qty FROM BiocareProductBundle:Item i JOIN BiocareProductBundle:Product p WHERE i.storage = :storage GROUP BY i.product');
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $query = $em->createQuery('SELECT distinct(p) as product, COUNT(i.id) as qty FROM BiocareProductBundle:Item i JOIN BiocareProductBundle:Product p WHERE i.storage = :storage GROUP BY i.product');
        $query->setParameter('storage', $id);
        $count = $query->getResult();
        return array(
            'count' => $count,
